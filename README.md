...
- **Swift `0.1.0-beta.4`** — until `0.1.0-beta.4` is published, enableNotifications() requests APNs registration but the documented one-call setup never forwards Apple’s registration callbacks, so it waits ten seconds and returns .noToken on a correctly provisioned physical iPhone. The SDK runtime works only after the host calls the internal PushTokenBridge methods that onboarding omits.
